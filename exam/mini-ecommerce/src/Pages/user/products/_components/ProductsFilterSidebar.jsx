...
export default function ProductsFilterSidebar({
  ...
  const handlePriceChange = (e) => {
    ...
    setFilters((prev) => ({
      ...prev,
      priceRange: [30000, Number(e.target.value)],
    }));
  };
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:relative top-0 lg:top-0 left-0 h-screen lg:h-auto w-72 lg:w-full bg-[#FFFCEB] lg:bg-transparent p-4 lg:p-0 z-50 lg:z-0 overflow-y-auto lg:overflow-visible transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"} lg:col-span-1 space-y-6`}
      >
        {/* Nút đóng cho Mobile */}
        <button
          onClick={onClose}
        ...
        <div className="bg-white comic-border shadow-comic p-5">
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2 text-xs font-bubble font-bold">
              {availableCategories.map((cat) => (
                <label
                  key={cat}
                  className="flex items-center justify-between p-1.5 hover:bg-yellow-50 cursor-pointer comic-border-sm border-transparent hover:border-black transition-all"
                >
          ...
          <div className="mb-6 pt-4 border-t-2 border-stone-900">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between gap-2 text-xs font-bubble font-bold">
                <div className="comic-border-sm px-2 py-1 bg-gray-50">
                  30.000₫
                </div>
                <span>-</span>
                <div className="comic-border-sm px-2 py-1 bg-comic-yellow">
          ...
          {/* Filter Group: Status */}
          <div className="mb-6 pt-4 border-t-2 border-stone-900">
            <h3 className="font-bubble font-black text-xs uppercase tracking-wider mb-3 bg-comic-purple text-black px-2 py-1 comic-border-sm inline-block shadow-comic-sm">
        ...
        <div className="bg-stone-900 text-white p-5 comic-border shadow-comic relative overflow-hidden">
          <div className="absolute -right-6 -bottom-6 text-7xl select-none opacity-20">
            ⚡
          </div>
          ...
          <p className="text-xs text-gray-300 mt-2 mb-4 font-bubble">
            Tặng túi Canvas One Piece + Móc chìa khóa kiếm Zoro cho đơn trên
            500k!
          </p>
