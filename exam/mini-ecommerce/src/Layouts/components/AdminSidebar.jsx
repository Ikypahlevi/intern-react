...
export default function AdminSidebar() {
  ...
  const navItems = [
    { path: "/admin/overview", icon: "fa-solid fa-chart-line", label: "Tổng quan" },
        ...
        <div className="p-4 border-b-[3px] border-black bg-comic-yellow flex flex-col gap-2 shadow-[0_3px_0px_#000]">
          <div className="flex items-center justify-between">
            <span className="font-comic text-xl uppercase tracking-tight text-black flex items-center gap-1">
              <i className="fa-solid fa-bolt text-red-600 font-bold"></i>SWOO! MANGA
            </span>
            <span className="bg-black text-comic-yellow font-comic text-[10px] px-1.5 py-0.5 border border-black shadow-[2px_2px_0px_#ba002a]">HQ v2</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="bg-white text-black border border-black font-comic text-[10px] px-2 py-0.5 uppercase tracking-wider font-bold shadow-[2px_2px_0px_#000]">ADMIN PORTAL ⚡</span>
          </div>
        </div>
        ...
        <div className="bg-white border-[2px] border-black p-2 shadow-[3px_3px_0px_#000] mb-2">
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-comic text-sm text-black truncate">{user?.name || 'Oda Master'}</p>
              <p className="font-comic text-[10px] text-red-600 uppercase font-bold">Super Admin ★</p>
            </div>
          </div>
        </div>
        <button 
